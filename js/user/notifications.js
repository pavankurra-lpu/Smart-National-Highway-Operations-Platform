...
const Notifications = {
    ...
    updateLiveTicker: () => {
        ...
        adminAlerts.slice(0, 5).forEach(a => {
            ...
            const color = a.type === 'EMERGENCY' ? '#f43f5e' : (a.type === 'TRAFFIC' ? '#f59e0b' : '#3b82f6');
            tickerItems.push(`
        ...
        if (tickerItems.length === 0) {
            tickerItems = [
                `<span><i class="fa-solid fa-satellite-dish" style="color:#3b82f6;"></i> GPS Telemetry Active · Real-time National Highway monitoring enabled.</span>`,
    ...
    renderCurrent: () => {
            ...
            if (dist <= 10) {
                ...
                proximityBadge = `
                    <div style="margin-top:4px; font-size:9.5px; color:#94a3b8;">
                        <i class="fa-solid fa-satellite-dish" style="color:#3b82f6;"></i> 10km Geofence Area • ${dist.toFixed(0)} km from current location
                    </div>
                `;
            }
        }
        ...
        if (advisory) {
            advisory.innerHTML = `
                ...
                ${proximityBadge}
                <div style="font-size:9px; color:#3b82f6; margin-top:5px; font-weight:600; display:flex; align-items:center; gap:6px;">
